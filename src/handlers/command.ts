...
export async function run_command(message: Message): Promise<void> {
    ...
    await core.plugins.forEach((plugin) => {
        const command_match = plugin.commands.find(
            (plugin_command: Command) => plugin_command.data.trigger == command_name.toLowerCase()
        );
        if (command_match) {
    ...
    if (!whitelist.includes(message.guild.id)) {
        return new Embed(message, {
            message: `This server is \`not whitelisted\`.
    ...
    if (user) {
        if (
            command.data.user_perms?.some((perm: PermissionString) => !user.permissions.has(perm)) &&
            message.author.id != developer
        ) {
