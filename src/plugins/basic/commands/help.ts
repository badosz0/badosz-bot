...
export = new Command({
    ...
    output: async ({ message, args = [] }: CommandInput) => {
        if (args[0]) {
            ...
            await core.plugins.forEach((plugin) => {
                const command_match = plugin.commands.find(
                    (plugin_command: Command) => plugin_command.data.trigger == args[0].toLowerCase()
                );
                if (command_match) {
            ...
            if (command) {
                return {
                    fields: [
                        ...
                        {
                            title: "Usage",
                            text: `${core.prefix}${command.data.trigger} ${
                                command.data.usage?.replace(/{prefix}/g, core.prefix) || ""
                            }`,
                        },
                    ],
                };
        ...
        core.plugins.forEach((plugin) => {
            ...
            help +=
                `**${plugin.name}** (${plugin.commands.length})\n` +
